...
class Simulation:
    ...
    stop_loss       = 1.5000 * 1e-2
    stop_gain       = 3.0000 * 1e-2
    ...
    def plot_analysis (self, day_prices, buy_x, buy_y, sell_x, sell_y, sma1, sma2, loband, hiband, mm_norm, mids):
        ...
        plt.plot(mids, label='mids', alpha=0.5, linestyle='dashed')
...
if __name__ == '__main__':
    ...
    for d in range(1,ndays):
        ...
        old_prices = sim.prices[(d-1)*T:d*T]
        ...
        for i in range(len(day_prices)):
            ...
            vec = day_prices[:i+1]#np.concatenate((old_prices,day_prices[:i+1]))
        ...
        # sim.plot_analysis(day_prices, buy_x, buy_y, sell_x, sell_y, sma20, sma50, loband, hiband, mm_norm, mids+mids)
