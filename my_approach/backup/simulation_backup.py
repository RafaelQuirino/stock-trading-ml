...
if __name__ == '__main__':
    ...
    for d in range(ndays):
        ...
        sma, loband, hiband = ts.BB(day_prices, winsize=50, stdmult=2.2)
        sma20 = ts.SMA(day_prices, winsize=20)
        sma50 = ts.SMA(day_prices, winsize=50)
        ...
        sma = ts.SMA(day_prices)
        ema = ts.EMA(day_prices)
        plt.plot(day_prices,label='prices')
        plt.plot(sma,label='sma')
        plt.plot(ema,label='ema')
        plt.plot(np.arange(0,len(ema)),np.full(len(ema),mid), label='mid')
        plt.legend()
        plt.show()
        continue
